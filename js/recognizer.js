...
(function (global) {
    ...
    const TEMPLATES = {
        ...
        // 閃電 — 各種 Z 形 (多種畫法變體, 提高辨識成功率)
        // 要求: 至少有 2 段相對水平 + 1 段斜線, 或純 4 點角點 Z
        lightning: [
            // 13 點密集 Z: 上橫 5 + 斜線 3 + 下橫 5
            [{x:0,y:0},{x:20,y:0},{x:40,y:0},{x:60,y:0},{x:80,y:0},
             {x:60,y:20},{x:40,y:40},{x:20,y:60},
             {x:0,y:80},{x:20,y:80},{x:40,y:80},{x:60,y:80},{x:80,y:80}],
            // 反 Z (從右上開始)
            [{x:80,y:0},{x:60,y:0},{x:40,y:0},{x:20,y:0},{x:0,y:0},
             {x:20,y:20},{x:40,y:40},{x:60,y:60},
             {x:80,y:80},{x:60,y:80},{x:40,y:80},{x:20,y:80},{x:0,y:80}],
            // 5 點 Z 骨架
            ...
            // 短橫 Z (玩家畫得匆忙, 上下不完整)
            [{x:10,y:5},{x:60,y:5},{x:40,y:40},{x:20,y:75},{x:70,y:75}],
            // 傾斜 Z (筆劃有點斜)
            [{x:5,y:10},{x:70,y:0},{x:40,y:40},{x:10,y:80},{x:75,y:70}],
            // 有尾巴的 Z (常見於潦草畫法)
            [{x:0,y:10},{x:25,y:10},{x:55,y:10},{x:80,y:10},{x:60,y:35},{x:35,y:55},{x:10,y:75},{x:30,y:80},{x:60,y:80},{x:80,y:80}]
        ],
        // 冰刺 — 閉合三角形 (避免與 teleport 的 V 型混淆, 必須閉合回起點)
        icespike: [
            ...
            // 更細三角 (變體)
            [{x:40,y:5},{x:10,y:75},{x:70,y:75},{x:40,y:5}]
        ],
        ...
        // 大地轟擊 — W / M 形 (鋸齒, 無水平段 — 與閃電 Z 區分)
        groundslam: [
            // 5 點 W: 2 個谷 + 3 個峰
            [{x:0,y:0},{x:20,y:80},{x:40,y:0},{x:60,y:80},{x:80,y:0}],
            // 5 點 M: 倒置
            [{x:0,y:80},{x:20,y:0},{x:40,y:80},{x:60,y:0},{x:80,y:80}],
            // 4 點簡化 W
            [{x:0,y:0},{x:25,y:80},{x:50,y:0},{x:75,y:80}],
            [{x:0,y:80},{x:25,y:0},{x:50,y:80},{x:75,y:0}],
            // 密集鋸齒 W (更多點, 強化鋸齒感)
            [{x:0,y:10},{x:12,y:75},{x:25,y:15},{x:40,y:75},{x:55,y:15},{x:70,y:75},{x:82,y:10}]
        ],
    ...
    global.Recognizer = {
        ...
        // 0.06 太嚴讓閃電常被拒; 降回 0.045 配合擴充 templates 使用
        MIN_MARGIN: 0.045
    };
})(window);
